...
def create_data(request):
    ...
    return render(request, 'polls/create_data.html')

def list_rockstar_games(request):
    rockstar_games = Game.objects.filter(company__name="Rockstar Games")
    context = {'rockstar_games': rockstar_games}
    return render(request, 'polls/list_rockstar_games.html', context)

def update_gta_sales(request):
    gta = get_object_or_404(Game, title="Grand Theft Auto V")
    gta.sales = 150000000
    gta.save()
    return render(request, 'polls/update_gta_sales.html')

def delete_last_of_us(request):
    last_of_us = get_object_or_404(Game, title="The Last of Us")
    last_of_us.delete()
    return render(request, 'polls/delete_last_of_us.html')
